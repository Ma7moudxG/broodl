...
export default function Dashboard() {
  ...
  async function handleSetMood(mood) {
    ...
    try {
      ...
      if (!newData?.[year]?.[month + 1]) {
        newData[year][month + 1] = {};
      }

      newData[year][month+1][day] = mood
      setData(newData)
  ...
  useEffect(() => {
    ...
    setData(userDataObj);
    console.log('current user data: ', userDataObj)
      ...
      <div className="flex items-stretch flex-wrap gap-4">
        {Object.keys(moods).map((mood, moodIndex) => {
          return (
            <button
              onClick={() => {
                const cuurentMoodValue = moodIndex + 1;
                handleSetMood(cuurentMoodValue);
              }}
              className={
            ...
            >
              ...
              <p
                className={
                  "text-indigo-500 pt-2 text-xs sm:text-sm md:text-base " +
                  fugaz.className
                }
              >
                {mood}
